	...
	public TscToken getNextToken() throws IOException {
		...
		if ((nextChar == '@') && strPos == 0) {
			...
			if ((lastToken.getContents().charAt(0) == '<') || ((lastToken.getContents().length() == 1) || cmdMap.containsKey(cmd) && !cmdMap.get(cmd).paramSep)) {
				...
				if (c.paramLen[paramNum] < 0) {
					//string param
					while (strPos < line.length()) {
						char ch = line.charAt(strPos);
						tokenStr += ch;
						strPos++;
						if (ch == '$') break;
					}
					nextToken = new TscToken(TscPane.STYLE_STRING, tokenStr, lineNum, character,
							character + tokenStr.length());
					character += tokenStr.length();
					argsRemaining--;
					paramNum++;
					...
					for (int i = 0; i < c.paramLen[paramNum]; i++) {
						try {
							...
							String earlyStyle = (c.CE_param[paramNum] == 'F') ? TscPane.STYLE_FLAG : TscPane.STYLE_NUM;
							nextToken = new TscToken(earlyStyle, tokenStr, lineNum, character,
									character + tokenStr.length());
							character += tokenStr.length();
							strPos += tokenStr.length();
							lastToken = nextToken;
					...
					String numStyle = (c.CE_param[paramNum] == 'F') ? TscPane.STYLE_FLAG : TscPane.STYLE_NUM;
					nextToken = new TscToken(numStyle, tokenStr, lineNum, character, character + c.paramLen[paramNum]);
			...
			if (nextChar == '<') {
				...
				cmd = tokenStr;
				paramNum = 0;
				boolean knownCmd = cmdMap.containsKey(cmd);
				if (knownCmd) {
				...
				String tagStyle = knownCmd ? TscPane.STYLE_TAG : TscPane.STYLE_UNKNOWN;
				nextToken = new TscToken(tagStyle, tokenStr, lineNum, character, character + 4);
				character += 4;
				strPos += 4;
